...
#################################################################
#mnist 是一个轻量级的类。它以Numpy数组的形式存储着训练、
# 校验和测试数据集。同时提供了一个函数，用于在迭代中获得minibatch，
mnist = input_data.read_data_sets("MNIST_data/", one_hot=True)

#占位符
#我们通过为输入图像和目标输出类别创建节点，来开始构建计算图。
#通过操作符号变量来描述这些可交互的操作单元，
#我们在TensorFlow运行计算时输入这个值。我们希望能够输入任意数量的MNIST图像，
...
# 存在在TensorFlow的用于描述交互性操作的图中。它们可以用于计算输入值，
# 也可以在计算中被修改。对于各种机器学习应用，一般都会有模型参数，可以用Variable表示。
W = tf.Variable(tf.zeros([784,10]))
b = tf.Variable(tf.zeros([10]))
#我们赋予tf.Variable不同的初值来创建不同的Variable：
# 在这里，我们都用全为零的张量来初始化W和b。因为我们要学习W和b的值，它们的初值可以随意设置。
#注意，W的维度是[784，10]，因为我们想要用784维的图片向量乘以它以得到一个10维的矩阵值向量，
...
cross_entropy = -tf.reduce_sum(y_*tf.log(y))#计算值与样本值
...
# 在这里更确切的说是随机梯度下降训练。在理想情况下，我们希望用我们所有的数据来进行每一步的训练，
# 因为这能给我们更好的训练结果，但显然这需要很大的计算开销。
# 所以，每一次训练我们可以使用不同的数据子集，这样做既可以减少计算开销，
# 又可以最大化地学习到数据集的总体特性。
